...
Scene::Scene(Scene const & source) // you pass in source by reference because you dont wanna make another copy 
{
...
Scene const & Scene::operator=(Scene const & source) // in case you make two successive assignments
{
	if(this!=&source)
	{
		for(int i=0; i < maxCapacity;i++)
