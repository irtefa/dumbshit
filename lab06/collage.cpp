...
void Collage::drawByCoordinatesSerial(PNG * result)
{
    ...
    for(int i = 0; i < width; ++i)
    {
        for(int j = 0; j < height; ++j)
        {
             ...
             *   and set it accordingly in the final collage image.
             * You may want to make use of the helper function inBounds() here
             */
			int final;
			...
			for(int layer = 0; layer < layers.size(); ++layer)
			{
				...
				bool in = inBounds(i - offsetW, j - offsetH, layers[layer].width(), layers[layer].height());
				
				if(in)
					final = layer;
			}

			*(*result)(i, j) = *(layers[final](i - coords[final].x, j - coords[final].y));	
        }
    }
}
...
void Collage::drawByCoordinatesParallel(PNG * result)
{
	...
	for(int i = 0; i < width; ++i)
    {
        for(int j = 0; j < height; ++j)
        {
            // TODO: insert your code here! It should be very similar (if not the
            //  same) to your serial code.
