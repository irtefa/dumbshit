...
#define SIZE_START 400
// number of random trees to generate for amortizing things
#define NUM_RAND 100
...
double findKeys( BST<int, int> const & tree, int size, int loopcount = 10) {
...
void testBuildTree( vector< BST<int, int> > & trees, Gnuplot const & g ) {
    ...
    trees.resize(NUM_TREES * 2 + NUM_TREES * NUM_RAND);
    int i = 0;
    int size = SIZE_START;
    for( int x = 0; x < NUM_TREES; x++ ) {
        ...
        fflush(stdout);
		// hack: make a bunch of random trees to try to amortize things for
		// the graph nicely
		double rand_time = 0;
		for( int num_rand = 0; num_rand < NUM_RAND; num_rand++ ) {
			p = buildTree( trees[i++], randm, size );
			rand_time += p.first;
		}
        cout << rand_time / NUM_RAND << "ms" << endl;
        rndFile << size << " " << rand_time / NUM_RAND << endl;
...
void testFindKeys( vector< BST<int, int> > const & trees, Gnuplot const & g ) {
    ...
    for( int x = 0; x < NUM_TREES; x++ ) {
		...
		double rand_time = 0;
		for( int num_rand = 0; num_rand < NUM_RAND; num_rand++ ) {
			p = findKeys( trees[i++], size, 50 );
			rand_time += p;
		}
        cout << rand_time / NUM_RAND << "ms (avg)" << endl;
        rndFile << size << " " << rand_time / NUM_RAND << endl;
