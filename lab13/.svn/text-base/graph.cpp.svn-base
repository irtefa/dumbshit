    ...
    for(it = graph.begin(); it != graph.end(); ++it)
    {
        ...
        for(size_t i = 0; i < adj.size(); ++i)
        {
            ...
            if(seen.find(make_pair(u, v)) == seen.end())
            {
                ...
                // mark the current u,v edge as seen by inserting into set
...
{
    assertExists(u, "getEdge");
    assertExists(v, "getEdge");
    assertConnected(u, v, "getEdge");
...
{
    assertExists(u, "setEdgeLabel");
    assertExists(v, "setEdgeLabel");
    assertConnected(u, v, "setEdgeLabel");
...
{
    assertExists(u, "getEdgeLabel");
    assertExists(v, "getEdgeLabel");
    assertConnected(u, v, "getEdgeLabel");
...
int Graph::getEdgeWeight(Vertex u, Vertex v) const
{
    ...
    assertExists(u, "getEdgeWeight");
    assertExists(v, "getEdgeWeight");
    assertConnected(u, v, "getEdgeWeight");
...
{   
    assertExists(u, "insertEdge");
    assertExists(v, "insertEdge");
...
{
    assertExists(u, "removeEdge");
    assertExists(v, "removeEdge");
    assertConnected(u, v, "removeEdge");
...
{
    assertExists(u, "setEdgeWeight");
    assertExists(v, "setEdgeWeight");
    assertConnected(u, v, "setEdgeWeight");
...
{    
    ...
    {
        cout << it->first;
        if(vertexLabels.find(it->first) != vertexLabels.end())
            cout << " \"" << vertexLabels.at(it->first) << "\"";
        cout << endl;
        ...
        for(git = it->second.begin(); git != it->second.end(); ++git)
        {
            ...
            if(vertexLabels.find(git->first) != vertexLabels.end())
                vertexColumn += " \"" + vertexLabels.at(git->first) + "\" ";
            cout << std::left << std::setw(24) << vertexColumn;
            string edgeColumn = "edge label = \"" + git->second.label + "\"";
            cout << std::left << std::setw(24) << edgeColumn;
            if(weighted)
...
void Graph::savePNG(string title) const
{
    ...
    for(it = graph.begin(); it != graph.end(); ++it)
    {
        ...
        for(git = it->second.begin(); git != it->second.end(); ++git)
        {
            neatoFile << "\t\"" << it->first;
            if(vertexLabels.find(it->first) != vertexLabels.end())
                neatoFile << " (" << vertexLabels.at(it->first) << ")";
            neatoFile << "\" -- \"" << git->first;
            if(vertexLabels.find(git->first) != vertexLabels.end())
                neatoFile << " (" << vertexLabels.at(git->first) << ")";
            neatoFile << "\"";
    ...
    string rmCommand = "rm -f " + filename;
